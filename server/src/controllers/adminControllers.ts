  ...
  try {
    ...
    if (orders.length > 0) {
      ...
      transformedData = await Promise.all(
        mostSoldArray.map(async (product: any) => {
      ...
      if (
        priceComparison.product1.totalSale < priceComparison.product2.totalSale
      ) {
...
export const getReviewCount = async (req: Request, res: Response) => {
  try {
    ...
    const reviews = await Promise.all(
      product?.reviews.map(async (reviewId: string) => {
    ...
    reviews.map((review: any) => {
      const item = frequencyMap.find(
        (item: any) => item.stars === review.stars,
      );
