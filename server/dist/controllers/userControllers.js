...
const updateUser = async (req, res) => {
    try {
        ...
        if (!usernameEquals) {
            ...
            if (potentialUser &&
                potentialUser._id.toString() !== (currentUser === null || currentUser === void 0 ? void 0 : currentUser._id.toString())) {
                return res
                    .status(400)
        ...
        res.status(500).json({ message: "A Problem Occurred" });
    }
};
...
const deleteUser = async (req, res) => {
    try {
        ...
        await productModel_1.default.updateMany({ reviews: { $in: deletedReviews.map((review) => review._id) } }, {
